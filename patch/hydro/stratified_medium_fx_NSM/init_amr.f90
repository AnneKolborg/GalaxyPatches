subroutine init_amr
   ...
   integer::noutput2,iout2,ifout_checkpoint2,ifout_plotfile2,ilun
  ...
  bisection_or_ordering:if(ordering /= 'bisection') then ! use usual ordering machinery
   ...
   if(nrestart>0)then
     ...
     if(nrestart_quad.eq.nrestart) allocate(bound_key_restart(0:ndomain))
      ...
      ! Read time variables
      read(ilun)noutput2,iout2,ifout_checkpoint2,ifout_plotfile2
      ...
      iout=iout2
      ifout_checkpoint=ifout_checkpoint2
      ifout_plotfile=ifout_plotfile2
      if(ifout_checkpoint.gt.nrestart+1) ifout_checkpoint=nrestart+1
      read(ilun)t
